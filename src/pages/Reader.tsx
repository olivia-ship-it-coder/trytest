...
import { BookOpen, Lightbulb, Bookmark, ArrowLeft, Columns2 } from 'lucide-react'
...
import CompareGridView from '@/components/TranslationView'
...
export default function Reader() {
  ...
  const [compareParaIndex, setCompareParaIndex] = useState(0)
  ...
  const bookTranslationIndices = readingBook?.id === 'sein-und-zeit' && currentSectionId
    ? sectionToTranslation[currentSectionId] ?? []
    : []

  const canCompare = !readingBook
    ? true
    : readingBook.id === 'sein-und-zeit' && bookTranslationIndices.length > 0

  const bookContentWidth = showTOC ? 'ml-64' : 'ml-0'
  const compareGridWidth = showTOC ? 'ml-64' : 'ml-0'

  const handleToggleCompare = () => {
    const next = !compareMode
    setCompareMode(next)
    if (next && readingBook?.id === 'sein-und-zeit') {
      setCompareParaIndex(0)
    }
  }

  const demoCompareIndices = useMemo(() => {
    return [compareParaIndex]
  }, [compareParaIndex])
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className={`transition-all duration-300 ${compareMode && !readingBook ? compareGridWidth : bookContentWidth}`}>
        {/* ====== 精选段落模式 (未选书) ====== */}
        {!readingBook && (
          <>
            <div className="mb-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 font-serif text-sm text-leather-500">
                    {compareMode ? '多译本对照模式' : '从书架选择一本图书开始阅读，或浏览下方精选段落'}
                  </p>
                </div>
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${
                    compareMode
                      ? 'bg-crimson-100 border-crimson-300 text-crimson-800'
                      : 'border-leather-200 text-leather-600 hover:bg-leather-100'
                  }`}
                >
                  <Columns2 size={14} />
                  {compareMode ? '关闭对照' : '译本对照'}
                </button>
              </div>
              {!compareMode && (
                <div className="mt-4 rounded-lg border border-leather-200 bg-leather-50/60 p-4">
            ...
            {compareMode ? (
              <div className="pr-4">
                <CompareGridView
                  paragraphIndices={demoCompareIndices}
                  currentIndex={compareParaIndex}
                  total={paragraphs.length}
                  onPrev={() => setCompareParaIndex((i) => Math.max(0, i - 1))}
                  onNext={() => setCompareParaIndex((i) => Math.min(paragraphs.length - 1, i + 1))}
                  title={paragraphs[compareParaIndex] ? `${paragraphs[compareParaIndex].chapter} · ${paragraphs[compareParaIndex].section}` : undefined}
                />
              </div>
            ) : (
        ...
        {readingBook && (
          <>
            ...
            ) : compareMode && bookTranslationIndices.length > 0 ? (
              <div className="pr-4">
                <CompareGridView
                  paragraphIndices={bookTranslationIndices}
                  currentIndex={compareParaIndex}
                  total={1}
                  onPrev={() => {}}
                  onNext={() => {}}
      ...
      {/* ====== 右侧概念面板 ====== */}
      {!compareMode && (
        <div className="hidden w-80 shrink-0 lg:block fixed right-6 top-20" style={{ height: 'calc(100vh - 7rem)' }}>
          <div className="rounded-xl border border-leather-200 bg-white/80 shadow-book overflow-hidden h-full">
